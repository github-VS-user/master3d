...
// Fetch order details by order number or name
function fetchOrderDetails(searchQuery) {
  showLoadingAnimation(); // Show loading animation
  fetch(`${serverUrl}/orders`)
    ...
    .then(data => {
      const order = data.orders.find(order => 
        ...
        order.destinataire.toLowerCase().includes(searchQuery.toLowerCase())
      );
...
document.getElementById('orderForm').addEventListener('submit', function (event) {
  event.preventDefault();
  const searchQuery = document.getElementById('orderNumber').value.trim();

  if (searchQuery === '') {
    alert('Veuillez entrer un numéro de commande ou un nom.');
    return;
  }

  fetchOrderDetails(searchQuery); // Fetch order details
});
